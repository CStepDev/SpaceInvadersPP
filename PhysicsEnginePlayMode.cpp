...
void PhysicsEnginePlayMode::detectPlayerCollisionsAndInvaderDirection(std::vector<GameObject>& objects, const std::vector<int>& bulletPositions)
{
	...
	for (it3; it3 != end3; it3++)
	{
		if (((*it3).isActive()) &&
			...
			((*it3).getTag() != "Player"))
		{
			// Get a reference to all the parts of the current game object that may be needed
			...
			if (currentTag == "invader")
			{
				...
				{
					// Currently no need to drop down and reverse from previous frame or any hits in this frame
					if (currentLocation.x >= WorldState::WORLD_WIDTH - currentSize.x)
					{
						// The invader has passed its furthest right position
