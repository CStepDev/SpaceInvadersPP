...
GameScreen::GameScreen(ScreenManagerRemoteControl* smrc, Vector2i res)
{
	...
	addPanel(m_GIH, std::move(guip), smrc);

	auto m_GOIH = std::make_shared<GameOverInputHandler>();
	auto gouip = std::make_unique<GameOverUIPanel>(res);
	addPanel(m_GOIH, std::move(gouip), smrc);

	m_ScreenManagerRemoteControl = smrc;

	float screenRatio = (float)(VideoMode::getDesktopMode().width / VideoMode::getDesktopMode().height);

	WorldState::WORLD_HEIGHT = (int)(WorldState::WORLD_WIDTH / screenRatio);

	m_View.setSize((float)WorldState::WORLD_WIDTH, (float)WorldState::WORLD_HEIGHT);
	m_View.setCenter(Vector2f((float)(WorldState::WORLD_WIDTH / 2), (float)(WorldState::WORLD_HEIGHT / 2)));
...
void GameScreen::initialise()
{
	...
	// Store all the bullet locations and initialise all the BulletSpawners in the invaders
	...
	for (it; it != end; it++)
	{
		...
		{
			std::static_pointer_cast<InvaderUpdateComponent>((*it).getFirstUpdateComponent())->initialiseBulletSpawner(getBulletSpawner(), i);
